    ...
    <table class="table table-striped table-hover table-condensed">
        ...
        <tr>
            ...
            <c:if test="${handAdd == null }">
                <th>操作
                    &emsp;
                    <button type="button" class="btn btn-xs btn-info" onclick="subjectAdd()">添加试题</button>
                </th>
            </c:if>
        </tr>
        </thead>
            ...
            <c:when test="${not empty subjects }">
                <c:forEach items="${subjects }" var="subject">
                    <tr id="tr-${subject.subjectId }">
                        ...
                        <td>
                            <c:if test="${fn:length(subject.optionA) > 5 }"> ${fn:substring(subject.optionA, 0, 5) }
                                <sub><a href="#"
                                        onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub> </c:if>
                            <c:if test="${fn:length(subject.optionA) <= 5 }"> ${subject.optionA }</c:if>
                        </td>
                        <td>
                            <c:if test="${fn:length(subject.optionB) > 5 }"> ${fn:substring(subject.optionB, 0, 5) }
                                <sub><a href="#"
                                        onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub> </c:if>
                            <c:if test="${fn:length(subject.optionB) <= 5 }"> ${subject.optionB }</c:if>
                        </td>
                        <td>
                            <c:if test="${fn:length(subject.optionC) > 5 }"> ${fn:substring(subject.optionC, 0, 5) }
                                <sub><a href="#"
                                        onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub> </c:if>
                            <c:if test="${fn:length(subject.optionC) <= 5 }"> ${subject.optionC }</c:if>
                        </td>
                        <td>
                            <c:if test="${fn:length(subject.optionD) > 5 }"> ${fn:substring(subject.optionD, 0, 5) }
                                <sub><a href="#"
                                        onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">[...]</a></sub> </c:if>
                        ...
                        <td>
                                <%--如果正确答案的不为空,则显示文本"**"--%>
                            <c:if test="${not empty subject.rightResult }">**</c:if>
                        </td>
                            ...
                            <c:if test="${handAdd == null }">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-info btn-sm"
                                            onclick="_iframe(1,'subject/${subject.subjectId }', 'subjects')">修改
                                    </button>
                                    <button type="button" class="btn btn-danger btn-sm delSubject"
                                            id="${subject.subjectId }">删除
                                    </button>
                                </div>
                            </c:if>
                        </td>
    ...
    <div>
        <ul class="pagination">
            ...
            <c:if test="${pageNow-1 > 0 }">
                <li>
                    <a href="subjects?startPage=${pageNow-1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">上一页</a>
                </li>
            </c:if>
            <c:forEach begin="${pageNow }" end="${pageNow+4 }" var="subPage">
                <c:if test="${subPage-5 > 0 }">
                    <li>
                        <a href="subjects?startPage=${subPage-5 }&handAdd=${handAdd }&examPaperId=${examPaperId }">${subPage-5 }</a>
                    </li>
                </c:if>
            </c:forEach>
            <c:forEach begin="${pageNow }" end="${pageNow+5 }" step="1" var="pageNo">
                ...
                <c:if test="${pageNo <= pageTotal }">
                    <c:if test="${pageNow == pageNo }">
                        <li class="active"><a
                                href="subjects?startPage=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }">${pageNo }</a>
                        </li>
                    </c:if>
                    <c:if test="${pageNow != pageNo }">
                        <li><a href="subjects?startPage=${pageNo }&handAdd=${handAdd }&examPaperId=${examPaperId }"
                               class="pageLink">${pageNo }</a></li>
                    </c:if>
                </c:if>
            </c:forEach>
            <c:if test="${pageNow+1 <= pageTotal }">
                <li>
                    <a href="subjects?startPage=${pageNow+1 }&handAdd=${handAdd }&examPaperId=${examPaperId }">下一页</a>
                </li>
            </c:if>
            ...
            <li>
                <div style="width: -1%;height:100%;float:right;">
                    <form action="subjects" id="scannerPageForm">
                        <input id="scannerPage" type="text" name="startPage" handAdd="${handAdd }"
                               examPaperId="${examPaperId }"
                               style="width: 35px;height: 33.2px;border: 1px solid gray;border-radius: 3px;"/>
                        <input class="btn btn-default goPage" type="submit" value="Go"
                               style="margin-left: -4px;margin-bottom: 3.3px;height: 33.6px;"/>
                    </form>
                </div>
            </li>
        </ul>
