...
<div class="container" style="margin-top: 150px;">
    ...
    <div style="width: 60%; height: 100%; float: left;">
            ...
            <dl subject="1" style="width:100%;">
                ...
                <c:if test="${esms != null }">
                    ...
                    <c:forEach items="${esms }" var="esm">
                        ...
                        <dd style="float: left;display: none; width: 100%;" index="${index }"
                            id="${esm.subject.subjectId }">
                            <div>
                            ...
                            <div>
                                ...
                                <c:if test="${esm.subject.subjectType == 0 }">
                                    <p class="aswer-p"><input class="aswer-option"
                                                              id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-A"
                                                              type="radio" name="chooseRight-${esm.subject.subjectId }"
                                                              value="A"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionA }</span></p>
                                    <p class="aswer-p"><input class="aswer-option"
                                                              id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-B"
                                                              type="radio" name="chooseRight-${esm.subject.subjectId }"
                                                              value="B"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionB }</span></p>
                                    <p class="aswer-p"><input class="aswer-option"
                                                              id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-C"
                                                              type="radio" name="chooseRight-${esm.subject.subjectId }"
                                                              value="C"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionC }</span></p>
                                    <p class="aswer-p"><input class="aswer-option"
                                                              id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-D"
                                                              type="radio" name="chooseRight-${esm.subject.subjectId }"
                                                              value="D"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionD }</span></p>
                                </c:if>
                                    <%-- 多选 --%>
                                <c:if test="${esm.subject.subjectType == 1 }">
                                    <p class="aswer-p subject-id-${esm.subject.subjectId }"
                                       data-sid="${esm.subject.subjectId }" data-type="checkbox"
                                       data-val="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-">
                                        <input class="aswer-option"
                                               id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-A"
                                               type="checkbox" name="chooseRight-${esm.subject.subjectId }" value="A"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionA }</span></p>
                                    <p class="aswer-p subject-id-${esm.subject.subjectId }"
                                       data-sid="${esm.subject.subjectId }" data-type="checkbox"
                                       data-val="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-">
                                        <input class="aswer-option"
                                               id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-B"
                                               type="checkbox" name="chooseRight-${esm.subject.subjectId }" value="B"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionB }</span></p>
                                    <p class="aswer-p subject-id-${esm.subject.subjectId }"
                                       data-sid="${esm.subject.subjectId }" data-type="checkbox"
                                       data-val="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-">
                                        <input class="aswer-option"
                                               id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-C"
                                               type="checkbox" name="chooseRight-${esm.subject.subjectId }" value="C"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionC }</span></p>
                                    <p class="aswer-p subject-id-${esm.subject.subjectId }"
                                       data-sid="${esm.subject.subjectId }" data-type="checkbox"
                                       data-val="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-">
                                        <input class="aswer-option"
                                               id="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-D"
                                               type="checkbox" name="chooseRight-${esm.subject.subjectId }" value="D"/>&nbsp;<span
                                            class="subjectOption">${esm.subject.optionD }</span></p>
                                </c:if>
                                    <%-- 简答题 --%>
                                <c:if test="${esm.subject.subjectType == 2 }">
                                    <p class="aswer-p subject-id-${esm.subject.subjectId }"
                                       data-sid="${esm.subject.subjectId }" data-type="textarea"
                                       data-val="${esm.examPaper.examPaperId }-${esm.subject.subjectId }-${index }-">
                                        <textarea class="aswer-textarea" name="chooseRight-${esm.subject.subjectId }"
                                                  style="width:100%;height:100px;"
                                                  onblur="textareaSubmit(this)"></textarea></p>
                                </c:if>
                            </div>
                                <%--获取 记录已选的试题信息 --%>
                                    ...
                                    <c:if test="${choose.subject.subjectId == esm.subject.subjectId }">
                                        <c:if test="${esm.subject.subjectType == 0 }">
                                            <span style="display: none;"
                                                  class="temp-subjectId">${choose.subject.subjectId },</span>
                                            <span style="display: none;"
                                                  class="temp-result">${choose.chooseResult },</span>
                                        </c:if>
                                        <c:if test="${esm.subject.subjectType == 1 }">
                                            <span style="display: none;"
                                                  class="temp-checkbox-subjectId">${choose.subject.subjectId }</span>
                                            <span style="display: none;"
                                                  class="temp-checkbox--result">${choose.chooseResult }</span>
                                        </c:if>
                                        <c:if test="${esm.subject.subjectType == 2 }">
                                            <span style="display: none;"
                                                  class="temp-textarea-subjectId">${choose.subject.subjectId }</span>
                                            <span style="display: none;"
                                                  class="temp-textarea--result">${choose.chooseResult }</span>
                                        </c:if>
                                    </c:if>
                                </c:forEach>
                            </c:if>
        ...
        <div style="width:100%; height:20%; margin-top: 170px;">
            ...
            <button style="float: left;margin-left: 10px;" id="nextSubject" type="button"
                    class="btn btn-default btn-lg">下一题
            </button>
        </div>
    </div>
    ...
    <div style="width: 38%; float: right;margin-top: -61px;">
        <div style="width:100%; height:63px;text-align: center;">
            ...
            <div style="float: right; width: 55%;height: 100%;line-height: 63px; text-align: left;">
                <a href="submit?studentId=${sessionScope.loginStudent.studentId }&examPaperId=${examPaperId }&classId=${classId }&gradeId=${gradeId }"
                   type="button" class="btn btn-default btn-sm" onclick="return confirm('确定提交吗?')">提交</a>
		...
		<button id="toggleFullScreen" class="fullscreen" style="display: none;" ></button>
...
<script src="https://cdn.jsdelivr.net/npm/screenfull@latest/dist/screenfull.min.js"></script>

<script src="${path }/reception/fullscreen/app.js"></script>
