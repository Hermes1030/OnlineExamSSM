                    ...
                    <c:when test="${fn:length(examPlans) > 0 }">
                        <c:forEach items="${examPlans }" var="ep">
                            <div class="col-md-4">
                                <div class="thumbnail">
                                    <img alt="300x200"
                                         src="${pageContext.request.contextPath }/images/reception/peitu1.jpg"/>
                                    <div class="caption">
                                        <h3>
                                                ${ep.examPaper.examPaperName }
                                        </h3>
                                        <p>考试科目: ${ep.course.courseName } &emsp;&emsp; 考试时长: <span
                                                id="examPaperTime${ep.examPaper.examPaperId }">${ep.examPaper.examPaperTime }</span>
                                            分钟</p>
                                        <p>题目数量: ${ep.examPaper.subjectNum } &emsp;&emsp;
                                            总分: ${ep.examPaper.examPaperScore } &emsp;&emsp;
                                            难易程度:
                                        ...
                                        <p>
                                            <button id="toggleFullScreen" class="fullscreen" style="background-color: #fff;border: 1px #fff;width: 335px;height: 50px;">
                                                <a class="btn btn-default btn-lg btn-block beginExam" id="${ep.examPaper.examPaperId }" style="display: flex;align-items: center;justify-content: center;height: 50px;background-color: #f1f1f1;padding: 10px;border-radius: 5px;" href="begin?classId=${ep.clazz.classId }&examPaperId=${ep.examPaper.examPaperId }&studentId=${sessionScope.loginStudent.studentId }&examTime=${ep.examPaper.examPaperTime }&gradeId=${gradeId }&beginTime=${ep.beginTime }">
                                                    进入考试
                                                </a>
                                            </button>
                                        </p>
