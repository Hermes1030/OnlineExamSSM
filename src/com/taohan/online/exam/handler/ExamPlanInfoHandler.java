...
public class ExamPlanInfoHandler {
    ...
    public ModelAndView getExamPlans(HttpServletRequest request) {
        ModelAndView model = new ModelAndView();
        model.setViewName("admin/examPlans");
        logger.info("获取待考考试信息");
		if(request != null){
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("classId", request.getSession().getAttribute("classId"));
			List<ExamPlanInfo> examPlans = examPlanInfoService.getExamPlans(map);
    ...
    /**
     * 添加待考信息到数据库。
     * 该方法接收一个考试安排对象作为参数，将其添加到考试安排信息列表中。
     * 添加成功后，重定向到考试安排列表页面。
     *
     * @param examPlan 考试安排记录信息对象，包含考试的详细信息。
     * @return 返回一个字符串，表示页面重定向的路径。
     */
    @RequestMapping(value = "examPlan", method = RequestMethod.POST)
    public String isAddExamPlan(ExamPlanInfo examPlan) {
        logger.info("添加待考记录：" + examPlan); // 记录添加的考试安排信息
        examPlanInfoService.isAddExamPlan(examPlan); // 调用服务层方法，实际添加考试安排信息
        return "redirect:examPlans"; // 重定向到考试安排列表页面
    }
